...
describe('Room terrain access', () => {
	for (const { label, pos, expectedMask } of roomTerrainCases) {
		test(`ROOM-TERRAIN-001 [${label}] Room.Terrain.get(x, y) returns the expected terrain mask`, async ({ shard, skip }) => {
			requireCapability(shard, skip, 'terrain', 'custom terrain setup is required for terrain mask assertions');
	...
	test('ROOM-TERRAIN-002 Room.Terrain.getRawBuffer() returns the room terrain as a 2500-byte Uint8Array', async ({ shard, skip }) => {
		requireCapability(shard, skip, 'terrain', 'custom terrain setup is required for terrain buffer assertions');
		await shard.createShard({
			players: ['p1'],
			rooms: [{ name: 'W1N1', rcl: 1, owner: 'p1', terrain: roomTerrainLayout }],
		});
	...
	test('ROOM-TERRAIN-003 Game.map.getRoomTerrain(roomName) provides equivalent terrain access to new Room.Terrain(roomName)', async ({ shard, skip }) => {
		requireCapability(shard, skip, 'terrain', 'custom terrain setup is required for terrain access equivalence assertions');
		await shard.createShard({
			players: ['p1'],
			rooms: [{ name: 'W1N1', rcl: 1, owner: 'p1', terrain: roomTerrainLayout }],
		});
