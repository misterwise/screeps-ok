...
describe('Intent overwrite and cancel', () => {
	test('INTENT-CREEP-005 repeated same-tick calls keep only the last intent', async ({ shard }) => {
	...
	test('INTENT-CREEP-006 cancelOrder removes a queued intent', async ({ shard }) => {
		await shard.createShard({
			players: ['p1', 'p2'],
			rooms: [
				{ name: 'W1N1', rcl: 1, owner: 'p1' },
	...
	test('INTENT-CREEP-006 cancelOrder returns ERR_NOT_FOUND when no intent queued', async ({ shard }) => {
