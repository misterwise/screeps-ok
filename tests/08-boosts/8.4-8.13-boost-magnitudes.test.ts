...
// ── BOOST-HARVEST-002: harvest boosts only apply during harvest() ──

describe('BOOST-HARVEST-002 harvest boosts only apply during harvest()', () => {
	test('BOOST-HARVEST-002 boosted WORK part repairs at normal REPAIR_POWER, not boosted', async ({ shard }) => {
		shard.requires('chemistry');
		await shard.ownedRoom('p1', 'W1N1', 6);
...
// ── BOOST-BUILD-002: build/repair boosts don't increase energy cost ──

describe('BOOST-BUILD-002 build/repair boosts do not increase energy cost', () => {
...
// ── BOOST-UPGRADE-002: upgrade boosts don't increase energy cost ──
...
// ── BOOST-CARRY-002: boosted CARRY parts still zero fatigue when empty ──

describe('BOOST-CARRY-002 boosted CARRY parts still contribute zero fatigue when empty', () => {
	test('BOOST-CARRY-002 empty boosted CARRY does not add weight for fatigue', async ({ shard }) => {
		shard.requires('chemistry');
		await shard.ownedRoom('p1', 'W1N1', 6);
