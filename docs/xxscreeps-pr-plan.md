...
`parity.json` currently registers 14 open parity gaps (44 tests) plus 5 intentional expected failures (10 tests). Test counts here are test rows as generated in `docs/status.md`, not catalog IDs — a matrix ID expands to many rows. The queue below is the agreed bug-fix focus; everything else is next-up, deferred, or blocked.

1. **`power-bank-shape-exposes-store-extension`** (SHAPE-NPC-003) — promoted 2026-07-25. The bank's loot is a public schema field (`store: powerBankStoreFormat` in the `mods/powerbank/powerbank.ts` struct) and `withOverlay` publishes schema fields, so the backing storage leaks onto the player surface beside the canonical `power` projection. Fix follows the mod's own convention — the same struct already has `'#nextDecayTime': 'int32'` — so rename the field to `'#store'` and update its three non-test readers (`power` getter, creation, `backend.ts` `renderStore`). ~4 lines, mod-local, and it removes an internal field from the public API rather than chasing exact shapes.
2. **`controller-unclaim-clears-safe-mode-cooldown`** (CTRL-UNCLAIM-005) — vanilla's unclaim SETS `safeModeCooldown` to `gameTime + SAFE_MODE_COOLDOWN` in non-novice rooms; xxscreeps's `release()` clears it. Genuine value bug, not covered by #318; needs its own upstream fix.
