...
Last refreshed: 2026-07-21 (pin `f01f0a23`).
...
[#329](https://github.com/laverdet/xxscreeps/pull/329) (memory: skip the save when Memory fails to serialize) OPEN — trimmed 2026-07-21 after laverdet's spec-chasing pushback to just the `memory-circular-ref-crash` fix (`crunch` under the serialization catch + cache drop). On merge: prune `memory-circular-ref-crash` (UNDOC-MEMJSON-005) at the consuming pin. The withdrawn halves became the intentional `memory-parsed-json-not-refreshed-across-ticks` acceptance — see the gaps doc.
...
`parity.json` currently registers 15 open parity gaps (29 tests) plus 3 intentional expected failures (8 tests). The queue below is the agreed bug-fix focus; everything else is next-up, deferred, or blocked.

1. **`structure-active-equal-distance-scan-order`** (STRUCTURE-ACTIVE-005) — break `isActive` ties between equal-distance same-type structures by vanilla object scan order.
2. **`game-object-json-room-tojson-null-crash`** residual (UNDOC-JSONOBJ-001) — parsed JSON snapshots omit nested `pos.x`/`pos.y`/`pos.roomName` for most object classes; include the same representative nested position fields as the live snapshots.
3. **`controller-unclaim-clears-safe-mode-cooldown`** (CTRL-UNCLAIM-005) — vanilla's unclaim SETS `safeModeCooldown` to `gameTime + SAFE_MODE_COOLDOWN` in non-novice rooms; xxscreeps's `release()` clears it. Genuine value bug, not covered by #318; needs its own upstream fix.
...
- **Memory subsystem residual** (1 gap, 1 test) — `memory-circular-ref-crash` is in flight as #329 (see PRs to track); remaining in `mods/meta/memory/memory.ts`: vanilla's first-access descriptor flip on `global.Memory` (`rawmemory-set-invalidates-parsed-memhack`).
