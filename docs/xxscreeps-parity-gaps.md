...
- Status: RE-TRIAGE — the engine gap this documented is closed.
- Decision: pin `38ee6170` splits strongholds into their own mod (xxscreeps#337/#330), which ships the five canonical bunker templates with reward levels, per-bunker defense behaviors, and a `#templateName` schema field the deploy processor reads. The capability stays `false` only because the adapter does not yet seed `#templateName` at placement or expose it through the invader-core snapshot; re-enabling it is adapter work, not an engine gap. `strongholdId` is still unmodeled.
