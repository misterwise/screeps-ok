...
Last refreshed: 2026-07-27 against pin `38ee6170`.

> When a gap moves to fixed-upstream, drop it from `parity.json` and remove the entry here. When a gap is accepted as an intentional shape divergence, move it out of `parity.json` into the adapter's `shapeDivergences` declaration (`adapters/xxscreeps/index.ts`) and into the Accepted divergences section below. Current status: 18 open gaps registered in `parity.json` plus five expected failures held intentional (`controller-my-reset-returns-undefined`, accepted 2026-07-20 per laverdet's undefined-shapes rulings; `memory-parsed-json-not-refreshed-across-ticks`, accepted 2026-07-21 per laverdet's #329 spec-chasing bar; `structure-active-equal-distance-scan-order`, `rawmemory-set-invalidates-parsed-memhack` and `power-bank-shape-exposes-store-extension`, all accepted 2026-07-25 — the first because neither engine's tie order is specified, the second because it asserts an engine mechanism whose observable consequences already pass, the third because upstream documents the member as an intentional xxscreeps extension and the rename that would remove it breaks ruin looting and blob migration). Four open gaps are PR-derived rows awaiting stable vanilla rather than xxscreeps bugs — see `docs/xxscreeps-pr-plan.md`. Four intentional shape divergences (flag `id`, body-part `boost`, controller `effects`, structure `effects`) are declared on the adapter. The `38ee6170` bump consumes xxscreeps#335/#338, which enable the power-creep mod and so register `LOOK_POWER_CREEPS`; `lookForAt` now rejects unregistered types with `ERR_INVALID_ARGS`, clearing `look-for-at-unknown-returns-empty` (ROOM-LOOK-006). It also lands the mods reorganization: constants move behind `xxscreeps:mods/constants`, Room and RoomObject interfaces move to hand-written `xx.d.ts` files, `mods/invader` splits into `mods/classic/invader` plus `mods/modern/stronghold`, `computeRoomMeta` moves to `mods/modern/sector/terrain.js`, and `mods/mmo` builds as its own tsc project. That stronghold mod (xxscreeps#337/#330) closes the `strongholdDeploy` capability gap, leaving one new open row (`stronghold-deploy-trigger-one-tick-late`) and a narrower `strongholdMetadata` skip. The power-creep mod closes `powerCreeps` itself: `placePowerCreep` now seeds the account roster and the room copy, leaving four new open rows (`power-creep-wins-movement-ties`, `power-creep-survives-nuke-impact`, `creep-attack-cannot-target-power-creep`, `power-creep-renew-stamps-next-tick-age`) and two narrower skips, `powerCreepAccountApi` and `powerEffects`. The same re-triage drops the intentional hold on `factory-power-effect-not-implemented`: that row never needed a live power, only `checkProduce` reading the factory's stored level. Full counts regenerate in `docs/status.md` on the next full run.
...
- Tests: MOVE-POWER-001
- Status: CONFIRMED at pin `38ee6170`; exposed by enabling `powerCreeps`.
- Cause: the power-creep move processor (`mods/mmo/powercreep/processor.ts`) commits at a fixed priority of `1`, the same value a `[MOVE]` creep computes from `basePriority = weight ? -weight / power : power` (weight 0 → power 1). Equal priorities are then broken by a coin flip — `sort((left, right) => (right.power - left.power) || (Math.random() - 0.5))` (`engine/processor/movement.ts:164`) — so the power creep takes the contested tile about half the time. Vanilla ranks candidates by `rate4 = moves / weight` with `moves` hardcoded to 0 for a power creep and `weight` floored to 1 (`processor/intents/movement.js:117-123`), and has no random term: its rate is 0 and it loses every tie, always.
- Plan: bid `-Infinity` for the power creep's own move, which takes the pairing out of the random tie-break entirely. Not `0`: xxscreeps's expression is two-regime — an unladen creep bids positive `power`, a laden one bids negative `-weight / power` (`mods/classic/creep/processor.ts:316`) — so a power creep at 0 loses to unladen creeps but beats every laden one, which vanilla never allows because its `rate4 = moves / weight` is positive for anything that can move. Vanilla's own numbers for a power creep (`moves = 0`, `weight` floored to 1) put through xxscreeps's expression give `-1 / 0`, and `-Infinity` is already the sentinel that file uses for a pulling creep. Note the safe-mode `-500` adjustment becomes a no-op at that value; confirm upstream whether that matters.
- Row note: MOVE-POWER-001 asserts the invariant over 16 independent ties in one room rather than one, so the coin flip cannot make it pass by luck (a single trial flipped run to run). Vanilla is deterministic and passes all 16.

### power-creep-survives-nuke-impact

- Tests: NUKE-IMPACT-008:powerCreepRoomwideRemoved
- Status: CONFIRMED at pin `38ee6170`; exposed by enabling `powerCreeps`.
- Cause: `PowerCreep` never overrides `#applyNukeImpact`, so it inherits the `RoomObject.prototype` no-op (`mods/modern/nuker/processor.ts:21`) and the room-wide destruction sweep at impact skips it. Vanilla zeroes every power creep in the room (`processor/intents/nukes/tick.js:25`) in the same loop that kills creeps and removes construction sites, dropped resources, tombstones and ruins.
- Plan: add the `#applyNukeImpact` override, mirroring `Creep`'s. The kill path behind it already exists.
