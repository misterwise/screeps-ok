...
The suite is governed by a small core set of companion docs:
...
- [`CONTRIBUTING.md`](CONTRIBUTING.md) — first-time setup, contributor workflow, and PR checklist
- [`docs/index.md`](docs/index.md) — doc map by audience and task
...
> [!NOTE]
> Every `npm test` invocation also runs `posttest`, which regenerates
> [`docs/status.md`](docs/status.md) and [`docs/coverage.html`](docs/coverage.html).
> See [`CONTRIBUTING.md`](CONTRIBUTING.md) for when those generated files should
> be committed.

### First local success
If you are setting up the repository for the first time, start with one adapter
...
# Build only the fast in-process adapter first
npm run setup:xxscreeps

# Prove the toolchain works with one representative test file
...
> as a pre-publish release gate — the package metadata is not yet treated as a
> stable downstream installation contract, and `"exports"` still resolves to
> raw TypeScript.
...
Use [`CONTRIBUTING.md`](CONTRIBUTING.md) for the shortest contributor path:

- first successful local run
- glossary for project-specific terms
- generated artifact policy
- PR checklist and validation commands

Use [`docs/index.md`](docs/index.md) if you want the full document map,
including which docs are public reference docs versus maintainer-only workflow
notes.
...
**`runPlayer`** — executes the code string in the engine. The last expression is the return value. Returns numbers (action return codes), strings, booleans, objects, or null. The call processes intents and advances the game by exactly one tick.
...
> `src/limitations.ts`, and capability-based skips use
